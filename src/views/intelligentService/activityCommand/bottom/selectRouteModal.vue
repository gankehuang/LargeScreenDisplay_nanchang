...
export default {
  ...
  methods: {
    mapInit (map) {
      ...
      const lineArr = []
      const _inNodeList = []
      const list = this.info.lineList // 路线数据数组
      ...
      this.inNodeList = _inNodeList
      lineList.push(this._createLine(lineArr))
    ...
    _createLine (pos) {
      ...
      // const polyEditor = new AMap.PolyEditor(this.map, marker)
      // polyEditor.open()
...
.map-operate {
  ...
  .modal-title {
    ...
    color: #fff;
    width: 100%;
    background-image: url("~@/assets/image/intelligentService/selectRouteModelTitle.png");
    padding: 14px 25px;
  }
  .content {
    position: relative;
  ...
  .modal-bottom {
    ...
    .btn-save {
      background: url("~@/assets/image/intelligentService/btn-save.png");
